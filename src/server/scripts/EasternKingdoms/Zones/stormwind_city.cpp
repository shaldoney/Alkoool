...
public:
    npc_archmage_malin() :
            CreatureScript("npc_archmage_malin") {
    }
...
enum eBartleby {
    FACTION_ENEMY = 168, QUEST_BEAT = 1640
};

class npc_bartleby: public CreatureScript {
public:
    npc_bartleby() :
            CreatureScript("npc_bartleby") {
    }
...
enum eDashel {
    QUEST_MISSING_DIPLO_PT8 = 1447, FACTION_HOSTILE = 168
};

class npc_dashel_stonefist: public CreatureScript {
public:
    npc_dashel_stonefist() :
            CreatureScript("npc_dashel_stonefist") {
    }
    ...
    bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
        ...
        if (pQuest->GetQuestId() == QUEST_MISSING_DIPLO_PT8) {
            ...
            CAST_AI(npc_dashel_stonefist::npc_dashel_stonefistAI, pCreature->AI())->AttackStart(
                    pPlayer);
        }
        return true;
    }
    ...
    struct npc_dashel_stonefistAI: public ScriptedAI {
        npc_dashel_stonefistAI(Creature *c) :
                ScriptedAI(c) {
            m_uiNormalFaction = c->getFaction();
        }
        ...
        void DamageTaken(Unit* pDoneBy, uint32 &uiDamage) {
            if (uiDamage > me->GetHealth()
                ...
                if (pDoneBy->GetTypeId() == TYPEID_PLAYER)
                    CAST_PLR(pDoneBy)->AreaExploredOrEventHappens(
                            QUEST_MISSING_DIPLO_PT8);
...
public:
    npc_lady_katrana_prestor() :
            CreatureScript("npc_lady_katrana_prestor") {
    }
...
public:
    ...
    struct npc_lord_gregor_lescovarAI: public npc_escortAI {
        npc_lord_gregor_lescovarAI(Creature* pCreature) :
        ...
        void WaypointReached(uint32 uiPointId) {
            ...
            case 16:
                ...
                if (Creature *pMarzon = me->SummonCreature(NPC_MARZON_BLADE, -8411.360352f, 480.069733f, 123.760895f, 4.941504f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000)) {
                    pMarzon->GetMotionMaster()->MovePoint(0, -8408.000977f,
                            468.611450f, 123.759903f);
        ...
        void DoGuardsDisappearAndDie() {
            ...
            me->GetCreatureListWithEntryInGrid(GuardList, NPC_STORMWIND_ROYAL,
                    8.0f);
            if (!GuardList.empty()) {
                for (std::list<Creature*>::const_iterator itr =
                        GuardList.begin(); itr != GuardList.end(); ++itr) {
...
public:
    npc_marzon_silent_blade() :
            CreatureScript("npc_marzon_silent_blade") {
    }
    ...
    struct npc_marzon_silent_bladeAI: public ScriptedAI {
        npc_marzon_silent_bladeAI(Creature* pCreature) :
        ...
        void MovementInform(uint32 uiType, uint32 /*uiId*/) {
            ...
            if (me->isSummon()) {
                if (Unit* pSummoner = CAST_SUM(me)->GetSummoner()) {
                    CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, CAST_CRE(pSummoner)->AI())->uiTimer =
                            2000;
                    CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, CAST_CRE(pSummoner)->AI())->uiPhase =
                            5;
...
public:
    npc_tyrion_spybot() :
            CreatureScript("npc_tyrion_spybot") {
    }
    ...
    struct npc_tyrion_spybotAI: public npc_escortAI {
        npc_tyrion_spybotAI(Creature* pCreature) :
                npc_escortAI(pCreature) {
        }
            ...
            if (uiPhase) {
                if (uiTimer <= uiDiff) {
                    ...
                    case 10:
                        if (Creature* pLescovar = me->FindNearestCreature(NPC_LORD_GREGOR_LESCOVAR, 10.0f)) {
                            if (Player* pPlayer = GetPlayerForEscort()) {
                                CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, pLescovar->AI())->Start(
                                        false, false, pPlayer->GetGUID());
                                CAST_AI(npc_lord_gregor_lescovar::npc_lord_gregor_lescovarAI, pLescovar->AI())->SetMaxPlayerDistance(
                                        200.0f);
                            }
                        }
                        me->DisappearAndDie();
    ...
    bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
        ...
        if (pQuest->GetQuestId() == QUEST_THE_ATTACK) {
            if (Creature* pSpybot = pCreature->FindNearestCreature(NPC_TYRION_SPYBOT, 5.0f, true)) {
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())->Start(
                        false, false, pPlayer->GetGUID());
                CAST_AI(npc_tyrion_spybot::npc_tyrion_spybotAI, pSpybot->AI())->SetMaxPlayerDistance(
                        200.0f);
            }
            return true;
        }
           ...
           while (uint32 eventId = events.ExecuteEvent())
           {
               switch (eventId)
               {
                   ...
                   case EVENT_HEROIC_LEAP:
                   {
                       ...
                       {
                           me->GetMotionMaster()->MoveJump(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), SPEED_CHARGE, 1);
