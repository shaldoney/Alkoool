...
AuraEffect::AuraEffect (Aura *base, uint8 effIndex, int32 *baseAmount, Unit *caster) :
        m_base(base), m_spellProto(base->GetSpellProto()), m_effIndex(effIndex), m_baseAmount(baseAmount ? *baseAmount : m_spellProto->EffectBasePoints[m_effIndex]), m_canBeRecalculated(true), m_spellmod(NULL), m_isPeriodic(false), m_periodicTimer(0), m_tickNumber(0)
{
...
int32 AuraEffect::CalculateAmount (Unit *caster)
{
    ...
    case SPELL_AURA_PERIODIC_DAMAGE:
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_ROGUE && m_spellProto->SpellFamilyFlags[0] & 0x100000)
        {
            ...
            float AP_per_combo[6] =
            { 0.0f, 0.015f, 0.024f, 0.03f, 0.03428571f, 0.0375f };
...
void AuraEffect::PeriodicTick (AuraApplication * aurApp, Unit * caster) const
{
    ...
    case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
    {
        ...
        if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
        {
            ...
            if (GetSpellProto()->Id == 1978)
                damage = int32((GetBase()->GetCaster()->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.4f + (damage * 15 / 3)) / 5);
    ...
    case SPELL_AURA_OBS_MOD_HEALTH:
    {
        ...
        sLog->outDetail("PeriodicTick: %u (TypeId: %u) heal of %u (TypeId: %u) for %u health inflicted by %u", GUID_LOPART(GetCasterGUID()), GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(), target->GetTypeId(), damage, GetId());
    ...
    case SPELL_AURA_PERIODIC_MANA_LEECH:
    {
        ...
        sLog->outDetail("PeriodicTick: %u (TypeId: %u) power leech of %u (TypeId: %u) for %u dmg inflicted by %u", GUID_LOPART(GetCasterGUID()), GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(), target->GetTypeId(), damage, GetId());
    ...
    case SPELL_AURA_OBS_MOD_POWER:
    {
        ...
        sLog->outDetail("PeriodicTick: %u (TypeId: %u) energize %u (TypeId: %u) for %u dmg inflicted by %u", GUID_LOPART(GetCasterGUID()), GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(), target->GetTypeId(), amount, GetId());
...
void AuraEffect::TriggerSpell (Unit *target, Unit *caster) const
{
    ...
    if (triggeredSpellInfo == NULL)
    {
        ...
        case SPELLFAMILY_GENERIC:
        {
            ...
            case 30427:
            {
                ...
                if (caster->GetTypeId() == TYPEID_PLAYER && triggerTarget->GetTypeId() == TYPEID_UNIT && triggerTarget->ToCreature()->GetCreatureInfo()->type == CREATURE_TYPE_GAS_CLOUD)
                {
    ...
    else
    {
        ...
        if (!c || (c && !sScriptMgr->OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()), triggerTarget->ToCreature())) || (c && !c->AI()->sOnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()))))
...
void AuraEffect::CleanupTriggeredSpells (Unit *target)
{
    ...
    if (m_spellProto->EffectApplyAuraName[GetEffIndex()] == SPELL_AURA_PERIODIC_TRIGGER_SPELL && uint32(GetSpellDuration(m_spellProto)) == m_spellProto->EffectAmplitude[GetEffIndex()])
        return;
...
void AuraEffect::HandleAuraSaleForGuild (AuraApplication const *aurApp, uint8 mode, bool apply) const
{
}
...
void AuraEffect::HandleAuraModEffectImmunity (AuraApplication const *aurApp, uint8 mode, bool apply) const
{
    ...
    if (!(apply) && target->GetTypeId() == TYPEID_PLAYER && (GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION))
    {
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
...
void AuraEffect::HandleAuraDummy (AuraApplication const *aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL)
    {
        ...
        {
            // Overpower
            if (caster && m_spellProto->SpellFamilyName == SPELLFAMILY_WARRIOR && m_spellProto->SpellFamilyFlags[0] & 0x4)
            {
            ...
            case 28835:          // Mark of Zeliek
                if (caster)          // actually we can also use cast(this, originalcasterguid)
                {
                    ...
                    case 6:
                        ...
                        break;
                    default:
                        damage = 20000 + 1000 * (GetBase()->GetStackAmount() - 7);
                        break;
                    }
                    if (damage)
        ...
        else
        {
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (m_spellProto->SpellFamilyFlags[1] & 0x0400 && aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL && GetEffIndex() == 0)
                {
                    ...
                    {
                        int32 chance = 0;
                        if (aurEff->GetSpellProto()->Id == 87099)
                            chance = 50;
                        if (aurEff->GetSpellProto()->Id == 87100)
                            chance = 100;
